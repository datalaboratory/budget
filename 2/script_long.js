...
var margin = {top: 60, right: 85, bottom: 20, left: 170},
...
// d3.dsv(';')("data/all_data.csv", function (error, data) {
...
//Данные по дням в году
    ...
    dataByGroup.forEach(function (group) {
        ...
        offsetSum += Math.max(group.maxWeekSum, 10000) + 2000;
    ...
    dataByWeek.forEach(function (group) {
        ...
        group.values.forEach(function (value) {
            ...
            if (value.category !== "Жильё/Аренда") {offset2 += value.value + 20;}; // 20 - отступ между тратами в столбце
...
//------------/Вычисление вспомогательных параметров
    ...
    // ...и тайтле
    d3.select("title")
    .text(t_format(total).replace(/,/g," ") + " руб., потраченных в 2012…2014 годах")
...
// Далее пошли описания функций для радио-кнопок
    function transitionStacked() {
        if (nocash.checked) {
            y0 = d3.scale.linear().domain([0, 30000*1.1])
            .range([0, height]),
            // Для вертикальных осей затрат и вертикального позиционирования на графике stacked (по неделям)
            y1 = d3.scale.linear().domain([0, 30000*1.1])
            .range([height, 0]);
        } else {
        ...
        var yAxis1 = d3.svg.axis()
            ...
            .scale(y1)
            .orient("right")
            .tickValues(function(){if (nocash.checked) {return [27042]} else {return [weekMax]};})
            .tickFormat(d3.format("d"));
        ...
        t.select(".xAxis")
            .attr("transform", "translate(-4,"+ (height - 3) +")");
        ...
        g.selectAll("rect")
            ...
            .attr("height", function (d) {
                if (nocash.checked && d.category === "Жильё/Аренда") { return 0;}
    ...
    function transitionMultiples() {
        ...
        if (nocash.checked) {
            var offsetConstant = height / 15.4,
                _y1 = d3.scale.linear().domain([0, 15000]).range([0, categories.length]), //domain
                _y2 = d3.scale.linear().domain([0, 15000]).range([categories.length, 0]);
            } else {
            var offsetConstant = height / 16.5,
                _y1 = d3.scale.linear().domain([0, 17000]).range([0, categories.length]), //domain
                _y2 = d3.scale.linear().domain([0, 17000]).range([categories.length, 0]);
            };
        ...
        g = t.selectAll(".group")
            .attr("transform", function (d,i) {
                if (nocash.checked && d.key === "Жильё/Аренда") {
                    return "translate(0,"+ 3000 +")";
                }
                else {return "translate(0,"+ (d.offsetSum/22000) * offsetConstant +")";}; // 43000 - надо регулировать чтобы убрать "наличку"
        ...
        g.select(".group-label").attr("y", 20);
        g.select(".group-sum").attr("y", 20);
    ...
    function transitionDays() {
    	...
    	data.forEach(function (d,i) {
    		...
    		for (var n = 0; n < data.length; n++) {
                ...
                if (nocash.checked && data[n].category === "Жильё/Аренда") {
        ...
        t.select(".xAxis")
            .attr("transform", "translate(-4,"+ (height + margin.bottom + 500) +")");
