...
//Данные по дням в году
    ...
    dataByGroup.forEach(function (group) {
        ...
        offsetSum += Math.max(group.maxWeekSum, 6000) + 1000;
    ...
    dataByWeek.forEach(function (group) {
        ...
        group.values.forEach(function (value) {
            ...
            if (value.category !== "наличные") {offset2 += value.value + 100;};
            offset += value.value + 100;
        })
    });
...
// //-----------/TEMP
    ...
    group.append("text")
        ...
        .attr("dy", ".35em")
        .style("fill", function (d) { return color[categories.indexOf(d.key)]; })
        .text(function (d) { return Math.floor(d.sum); });
    ...
    d3.select("body")
        ...
        .html('Визуализация <a href="http://datalaboratory.ru">Лаборатории данных</a>, расходы из личного кабинета <a href="http://tcsbank.ru">банка ТКС</a></a>');
...
// Далее пошли описания функций для радио-кнопок
    function transitionStacked() {
        ...
        g.selectAll("rect")
            ...
            .attr("height", function (d) {
                if (nocash.checked && d.category === "наличные") { return 0;}
    ...
    function transitionMultiples() {
        ...
        if (nocash.checked) {
            ...
            var offsetConstant = height / 15,
                _y1 = d3.scale.linear().domain([0, 10000]).range([0, 15]),
                _y2 = d3.scale.linear().domain([0, 10000]).range([15, 0]);
            };


        g = t.selectAll(".group")
            .attr("transform", function (d,i) {
                if (nocash.checked && d.key === "наличные") {
        ...
        g.select(".group-label").attr("y", function() {if (nocash.checked) {return 16;} else {return 10;}});
        g.select(".group-sum").attr("y", function() {if (nocash.checked) {return 16;} else {return 10;}});
    ...
    function transitionDays() {
    	...
    	data.forEach(function (d,i) {
    		...
    		for (var n = 0; n < data.length; n++) {
                ...
                if (nocash.checked && data[n].category === "наличные") {
        ...
        t.select(".xAxis")
            .attr("transform", "translate(-6,"+ (height + margin.bottom + 500) +")");
